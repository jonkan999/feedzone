...
export function renderProductCard(product) {
  ...
  return `
    <div class="product-card bg-white hover:bg-gray-100 border border-gray-200 hover:border-gray-300 rounded-lg overflow-hidden transition-all duration-300 cursor-pointer" data-product-id="${
      product.id
    }" data-product-link>
    ...
    product.name
      ...
      <div class="p-8">
        <h3 class="font-bold text-sm mb-1 text-gray-900 line-clamp-2">${
          product.name
        }</h3>
        ...
        <div class="flex items-center justify-between">
          <span class="text-lg font-bold text-gray-900">${product.price.toFixed(
            2
          )} SEK</span>
      ...
      if (productCard) {
        ...
        if (controlsDiv) {
          const quantityButtons =
            controlsDiv.querySelector(".quantity-buttons");
          if (quantityButtons) {
  ...
  container.addEventListener("click", (e) => {
    ...
    if (kopBtn) {
      ...
      const productPrice = parseFloat(
        controlsDiv.getAttribute("data-product-price") || "0"
      );
